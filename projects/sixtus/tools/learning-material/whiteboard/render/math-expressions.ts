...
export function renderMathExpressionsDrawing(
  ...
  const layout = (node: MathNode, s: number): Box => {
    switch (node.kind) {
      ...
      case "align": {
        const table = node.rows.map((row) =>
          row.map((cell) => layout(cell, s))
        );
        const cols = Math.max(0, ...table.map((row) => row.length));
        const widths = Array.from(
          { length: cols },
          (_, i) => Math.max(0, ...table.map((row) => row[i]?.width ?? 0)),
        );
        ...
        table.forEach((row, ri) => {
          const ascent = Math.max(0, ...row.map((cell) => cell.ascent));
          const descent = Math.max(0, ...row.map((cell) => cell.descent));
          if (ri > 0) baseline += prevDescent + rowGap + ascent;
          let x = 0;
          row.forEach((cell, ci) => {
            const offset = ci % 2 === 0 ? widths[ci] - cell.width : 0;
            parts.push(move(cell, x + offset, baseline));
            x += widths[ci];
          });
          prevDescent = descent;
        });
        return box(
          parts,
          totalWidth,
          table[0] ? Math.max(0, ...table[0].map((cell) => cell.ascent)) : 0,
          baseline + prevDescent,
        );
      }
    }
  };
