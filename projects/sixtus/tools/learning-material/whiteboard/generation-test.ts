...
import { executeWhiteboardWith, generatePreparedBoard, InvalidGeneratedOutput, type FigureGenerator } from "./generation.ts";
import type { WhiteboardPlan } from "./index.ts";
import type { WhiteboardFigureContent } from "./schema.ts";
import { renderWhiteboardSvg, composeWhiteboard } from "./render/index.ts";
...
const plan: WhiteboardPlan = {
  title: null,
  figurePlans: [1, 2].map((n) => ({ type: "math_expressions", id: `figure-${n}`, anchor: n === 1 ? null : "figure-1", side: n === 1 ? null : "bottom", instructions: "Show x = 8." })),
};
const good = (id: string): WhiteboardFigureContent => ({ type: "math_expressions", id, title: null, annotations: [{ type: "box", targetIds: [`${id}.final.expression`], content: null }], expressions: [{ id: "final", latex: "x=8" }] });
const noTitle = () => { throw new Error("Unexpected title correction"); };
...
Deno.test("only the failed figure is repaired, with exact issues and original output", async () => {
  ...
  const generateFigure: FigureGenerator = (p, _context, repair) => {
    ...
    if (p.id === "figure-2" && count === 1) return Promise.resolve({ ...good(p.id), expressions: [{ id: "final", latex: "∵ x=8" }] });
    if (count === 2) {
      assert(repair);
  ...
  const result = await generatePreparedBoard(plan, {}, { generateFigure, correctTitle: noTitle });
  assertEquals([...calls], [["figure-1", 1], ["figure-2", 2]]);
  assertEquals(result.prepared.map((figure) => figure.id), ["figure-1", "figure-2"]);
  assertEquals(result.rendered, renderWhiteboardSvg(result.spec));
  const base = result.prepared[0].base;
  assertEquals(composeWhiteboard(result.spec, result.prepared), result.rendered);
...
Deno.test("unknown annotation target diagnostics include compatible visible targets", async () => {
  ...
  await generatePreparedBoard(single, {}, {
    generateFigure: (p, _context, repair) => {
      if (++calls === 1) return Promise.resolve({ ...good(p.id), annotations: [{ type: "box", targetIds: [`${p.id}.final.result`], content: null }] });
      assertEquals(repair?.issues[0].code, "UNKNOWN_TARGET");
      assert(repair?.issues[0].availableTargetIds?.includes(`${p.id}.final.expression`));
      return Promise.resolve(good(p.id));
...
Deno.test("schema failures repair once and corrected output is revalidated", async () => {
  let calls = 0;
  const error = await assertRejects(() => generatePreparedBoard({ ...plan, figurePlans: plan.figurePlans.slice(0, 1) }, {}, {
    generateFigure: (p, _context, repair) => {
      calls++;
      if (calls === 1) throw new InvalidGeneratedOutput("broken JSON", renderError("INVALID_SPEC", "Invalid JSON").issues);
...
Deno.test("exhausted repairs are aggregated and prevent upload", async () => {
  ...
  const error = await assertRejects(() => executeWhiteboardWith({ goal: "Show an equation" }, {
    generatePlan: () => Promise.resolve(plan),
    generateFigure: (p) => { calls++; return Promise.resolve({ ...good(p.id), expressions: [{ id: "final", latex: "∵" }] }); },
    correctTitle: noTitle,
    upload: () => { uploads++; return Promise.resolve("unused"); },
  }), WhiteboardRenderError);
  assertEquals(calls, 4);
  assertEquals(uploads, 0);
  assertEquals(error.issues.map((issue) => issue.figureId), ["figure-1", "figure-2"]);
});

Deno.test("unexpected generation errors are not model-retried or exposed", async () => {
  let calls = 0;
  const error = await assertRejects(() => generatePreparedBoard({ ...plan, figurePlans: plan.figurePlans.slice(0, 1) }, {}, {
    generateFigure: () => { calls++; throw new Error("private provider payload"); }, correctTitle: noTitle,
  }), WhiteboardRenderError);
...
Deno.test("board title correction reuses every prepared figure", async () => {
  ...
  const result = await generatePreparedBoard({ ...plan, title: "😀" }, {}, {
    generateFigure: (p) => { figures++; return Promise.resolve(good(p.id)); },
...
Deno.test("unrenderable title correction is rejected after one attempt", async () => {
  let titles = 0;
  await assertRejects(() => generatePreparedBoard({ ...plan, title: "😀" }, {}, {
    generateFigure: (p) => Promise.resolve(good(p.id)),
    correctTitle: () => { titles++; return Promise.resolve("😀"); },
  }), WhiteboardRenderError);
  assertEquals(titles, 1);
});
...
Deno.test("supplied specs render deterministically without any model call", async () => {
  ...
  const spec = { title: null, figures: [{ ...good("figure-1"), id: "figure-1", anchor: null, side: null }] };
  const result = await executeWhiteboardWith(spec, {
    generatePlan: unexpected, generateFigure: unexpected, correctTitle: unexpected,
    upload: (svg) => { uploads++; assertEquals(svg, renderWhiteboardSvg(spec).svg); return Promise.resolve("https://example.test/board.svg"); },
  });
...
Deno.test("render failures return 422 only for supplied content", () => {
  const error = renderError("UNSUPPORTED_GLYPH", "Unsupported glyph", { figureId: "figure-1", elementId: "final" });
