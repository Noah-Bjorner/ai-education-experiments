// Adapted from Tegaki (MIT). See LICENSE and README.md in this directory.
...
function traceChain(
  ...
  while (true) {
    ...
    if (unvisited.length === 0) {
      ...
      const visitedJunction = neighbors.find((n) =>
        visited[n.y * width + n.x] &&
        degree(n.x, n.y, skeleton, width, height) >= 3
      );
    ...
    if (chain.length >= 2 && unvisited.length > 1) {
      ...
      const { dirX, dirY } = estimateDirection(
        chain,
        cx,
        cy,
        lookback,
      ...
      next = unvisited.find((n) =>
        degree(n.x, n.y, skeleton, width, height) <= 2
      ) ?? unvisited[0]!;
    }
...
function peekAhead(
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const forward = neighbors.filter((n) =>
      (n.x !== px || n.y !== py) && !visited[n.y * width + n.x]
    );
...
function shouldStopAtJunction(
  ...
  for (let i = 0; i < branchDirs.length && !hasStraightPair; i++) {
    for (let j = i + 1; j < branchDirs.length; j++) {
      const cos = branchDirs[i]!.dx * branchDirs[j]!.dx +
        branchDirs[i]!.dy * branchDirs[j]!.dy;
  ...
  for (let i = 1; i < polyline.length - 1; i++) {
    ...
    if (result.length >= 3) {
      ...
      if (predLen > 0) {
        ...
        if (skipLen > 0 && toCurrLen > 0) {
          ...
          const cosThrough = (dirX * toCurrX + dirY * toCurrY) /
            (predLen * toCurrLen);
    ...
    if (result.length >= 2) {
      ...
      if (inLen > 0 && withLen > 0 && withoutLen > 0) {
        ...
        const cosWithout = (inX * withoutX + inY * withoutY) /
          (inLen * withoutLen);
...
export function traceAndSimplify(
  ...
  const smoothed = mergedPolylines.map((p) =>
    smoothJunctionKinks(p, lookback, curvatureBias)
  );

  // Compute spur length threshold: proportional to bitmap size, but capped so tiny glyphs aren't fully erased
  const effectiveSpurMin = spurMinLength ??
    Math.min(Math.round(Math.max(width, height) * SPUR_LENGTH_RATIO), 10);
