 ...
 * Paste `goal` into the whiteboard tool. `expectedTypes` / `expectedSide`
 * are tester hints only — do not put type names in the goal itself.
 */
export type WhiteboardPromptSide = "top" | "left" | "right" | "bottom" | null;
...
export type WhiteboardPromptCase = {
  ...
  expectedSide: WhiteboardPromptSide;
  goal: string;
};
