...
import { contextualize, renderError, type RenderIssue } from "./render/issues.ts";
import { executeWhiteboardWith, generatePreparedBoard, generatePreparedFigure, InvalidGeneratedOutput, REPAIR_INSTRUCTIONS, type GenerationContext, type RepairRequest } from "./generation.ts";
...
async function generateFigureOutput(
  ...
  try {
    const { output } = await generateText({
      ...
      prompt,
      output: Output.object<WhiteboardFigureContent>({ schema, name: `${figure.type}_spec` }),
    });
    return output;
  } catch (cause) {
    if (NoObjectGeneratedError.isInstance(cause) && typeof cause.text === "string" && cause.finishReason !== "content-filter") {
      let output: unknown = cause.text;
      try { output = JSON.parse(cause.text); } catch { /* Retain raw invalid JSON for correction. */ }
      const parsed = schema.safeParse(output);
      const error = parsed.success
        ? renderError("INVALID_SPEC", "The generated figure could not be decoded as structured output.", { stage: "validation" })
...
async function correctBoardTitle(title: string, context: GenerationContext, issues: RenderIssue[]): Promise<unknown> {
  const { output } = await generateText({
    ...whiteboardGenerateTextOptions(context.mode),
    maxRetries: 0,
    system: "Rewrite the board title using Shantell-supported plain text while preserving its meaning. Return a nonempty title, never omit it. Do not change or regenerate any figures.",
    prompt: JSON.stringify({ title, goal: context.goal, issues }),
    output: Output.object({ schema: z.object({ title: z.string().trim().min(1) }), name: "whiteboard_title_correction" }),
  });
  return output.title;
}
...
export async function generateWhiteboardFigureSpec(
  ...
  return (await generatePreparedFigure(plan, context, generateFigureOutput, figureOptions({}, 0))).spec;
}

/** Generate, prepare, and validate the complete board while preserving plan order. */
export async function generateWhiteboardSpec(
  plan: WhiteboardPlan, goal?: string, mode?: WhiteboardMode,
): Promise<WhiteboardSpec> {
  return (await generatePreparedBoard(plan, { goal, mode }, {
    generateFigure: generateFigureOutput, correctTitle: correctBoardTitle,
  })).spec;
}
...
if (import.meta.main) {
  ...
  const { spec, rendered: { svg } } = await generatePreparedBoard(plan, { goal, mode }, {
    generateFigure: generateFigureOutput, correctTitle: correctBoardTitle,
