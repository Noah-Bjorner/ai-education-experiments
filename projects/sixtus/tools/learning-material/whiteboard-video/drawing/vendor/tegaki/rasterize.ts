...
import type { BBox, Point } from "../../geometry.ts";
...
export function rasterize(
  ...
  const padX = Math.max(
    bboxW * BITMAP_PADDING,
    Math.max(bboxW, bboxH) / resolution * 3,
  );
  const padY = Math.max(
    bboxH * BITMAP_PADDING,
    Math.max(bboxW, bboxH) / resolution * 3,
  );
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      while (
        nextIdx < intersections.length &&
        intersections[nextIdx]!.x <= pixelCenter
      ) {
