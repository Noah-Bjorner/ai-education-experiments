...
const SHARED_DOCUMENT_INSTRUCTIONS = [
  ...
  "- After each sourced claim, add <citation ref=\"SOURCE_ID\" /> using an exact source.id from gatherContext",
  "- Reuse the same id when multiple claims come from the same source",
  "- Do not invent ids, titles, URLs, or a Sources section; the application generates the bibliography from cited ids",
  "- If no returned source supports a claim, omit it or state uncertainty clearly",
  "",
  "## Output",
...
const DOCUMENT_TYPE_INSTRUCTIONS: Record<DocumentType, string> = {
  ...
  deepResearch: [
    ...
    "- Support factual claims with <citation ref=\"SOURCE_ID\" /> tags from gatherContext; do not write a Sources section.",
    "Tool policy:",
