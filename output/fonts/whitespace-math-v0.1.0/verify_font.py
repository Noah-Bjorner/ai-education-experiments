...
def verify(source, artwork, output):
    ...
    metrics = json.loads((output / 'whitespace-math-metrics.json').read_text())
    ...
    for extension in ['ttf', 'woff2']:
        font_path = output / f'WhitespaceMath-Medium.{extension}'
        ...
        assert font['name'].getDebugName(16) == 'Whitespace Math'
        assert font['name'].getDebugName(17) == 'Medium'
        assert font['name'].getDebugName(6) == 'WhitespaceMath-Medium'
    ...
    ttf = str(output / 'WhitespaceMath-Medium.ttf')
    caption = ImageFont.truetype('/System/Library/Fonts/Helvetica.ttc', 20)
    heading = ImageFont.truetype(ttf, 58)
    draw.text((42, 24), 'Whitespace Math Medium', font=heading, fill='#252726')
    rows = [
        ...
        ('Calculus', '∇f   ∂f   ∫ f(x)   ∬ f(x,y)   ∭ f(x,y,z)   ∮ f(z)'),
    ...
    report = {'status': 'passed', 'mapped_characters': 748, 'added_characters': 93,
              'preserved_original_glyphs': len(original_order),
              'all_original_outlines_and_metrics_preserved': True,
